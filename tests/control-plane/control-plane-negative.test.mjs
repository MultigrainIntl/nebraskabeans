...
add('A30-active-with-trailing-space-value-valid',0,()=>{let raw=fs.readFileSync(p('project-control/REQUIREMENTS.yaml'),'utf8');raw=raw.replace('status: ACTIVE','status: "ACTIVE "');fs.writeFileSync(p('project-control/REQUIREMENTS.yaml'),raw);const r=req();active(r).status='ACTIVE';saveReq(r);});
