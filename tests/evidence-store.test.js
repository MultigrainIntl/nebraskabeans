...
const row=(id,issue,value,source='nass',classification='OBSERVED')=>({id,variable:'yield_lb_ac',geography_id:'NE',valid_time:'2026-08-01T00:00:00Z',issue_time:`${issue}T12:00:00Z`,classification,source_id:source,value,units:'lb/ac',method:'published forecast'});
...
test('unregistered confidence thresholds fail closed',()=>{const c=confidence([{name:'geography',score:.9,weight:2}]);assert.equal(c.label,'UNKNOWN');assert.equal(c.score,null)});
test('registered confidence model exposes identity and weaknesses',()=>{const contract={id:'test-confidence',version:'1',thresholds:{high:.75,moderate:.5},weak_below:.6};const c=confidence([{name:'geography',score:.9,weight:2},{name:'irrigation',score:.3,weight:1},{name:'satellite',score:.8,weight:1}],contract);assert.equal(c.label,'MODERATE');assert.equal(c.model_id,'test-confidence');assert.deepEqual(c.weaknesses,['irrigation'])});
