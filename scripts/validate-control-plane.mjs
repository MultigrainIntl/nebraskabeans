...
function stringList(value, where) { if (!Array.isArray(value) || value.some(v => typeof v !== 'string' || !v.trim())) { fail(`CONTROL-SCHEMA-007 ${where} must be a list of non-empty strings`); return []; } return value; }
function readYamlDirectory(rel, label) { const dir=path.join(root,rel); let names; try { names=fs.readdirSync(dir,{withFileTypes:true}); } catch(error){ fail(`CONTROL-IO-002 cannot read ${label} directory ${rel}: ${error.message}`); return []; } const files=names.filter(e=>e.isFile()&&e.name.endsWith('.yaml')).map(e=>e.name).sort(); if(!files.length) fail(`CONTROL-IO-003 ${label} directory contains no YAML records`); return files.map(name=>({name,record:readYaml(path.join(rel,name))})); }
...
if(lock){keysOnly(lock,new Set(['schema_version','gate','status','approval_ref','authorized_by','authorized_at','scope','stop_condition']),'EXECUTION_LOCK');requireKeys(lock,['schema_version','gate','status','approval_ref','authorized_by','authorized_at','scope','stop_condition'],'EXECUTION_LOCK');keysOnly(lock.scope,new Set(['allow','prohibit']),'EXECUTION_LOCK.scope');stringList(lock.scope?.allow,'EXECUTION_LOCK.scope.allow');stringList(lock.scope?.prohibit,'EXECUTION_LOCK.scope.prohibit');}
...
const approvals=[]; const approvalById=new Map();
for(const {name,record:a} of approvalFiles){if(!a)continue;keysOnly(a,new Set(['schema_version','id','authority','authorization_type','gate','authorized_at','base_sha','candidate_sha','scope','notes','source']),`approval.${name}`);requireKeys(a,['schema_version','id','authority','authorization_type','gate','authorized_at','base_sha','candidate_sha','scope'],`approval.${name}`);keysOnly(a.scope,new Set(['allow','prohibit']),`approval.${name}.scope`);stringList(a.scope?.allow,`approval.${name}.scope.allow`);stringList(a.scope?.prohibit,`approval.${name}.scope.prohibit`);if(a.source)keysOnly(a.source,new Set(['type','authority','statement']),`approval.${name}.source`);if(a.authority!=='GAJ')fail(`CONTROL-APPROVAL-002 ${name} authority must be GAJ`);if(!nonEmpty(a.id)||!nonEmpty(a.gate)||!nonEmpty(a.authorized_at))fail(`CONTROL-APPROVAL-003 ${name} missing identity/gate/time`);if(approvalById.has(a.id))fail(`CONTROL-APPROVAL-006 duplicate approval id ${a.id}`);approvalById.set(a.id,a);approvals.push(a);}
...
if(requirements&&state&&lock){
  ...
  const activeReq=requirements.requirements.find(r=>r.id===state.active_gate);const matching=approvals.filter(a=>a.gate===state.active_gate);if(!matching.length)fail(`CONTROL-APPROVAL-004 active gate ${state.active_gate} has no matching approval`);
  if(!nonEmpty(lock.approval_ref)) fail('CONTROL-APPROVAL-007 execution lock missing approval_ref');
  const bound=approvalById.get(lock.approval_ref);
  if(!bound) fail(`CONTROL-APPROVAL-008 execution lock approval_ref ${lock.approval_ref} is not resolvable`);
  else {
    if(bound.gate!==lock.gate) fail(`CONTROL-APPROVAL-009 approval ${bound.id} gate ${bound.gate} != lock gate ${lock.gate}`);
    if(lock.authorized_by==='GAJ'&&bound.authority!=='GAJ') fail(`CONTROL-APPROVAL-010 lock claims GAJ authorization but ${bound.id} authority is ${bound.authority}`);
    const approvedAllow=new Set(stringList(bound.scope?.allow,`approval.${bound.id}.scope.allow`));
    for(const entry of stringList(lock.scope?.allow,'EXECUTION_LOCK.scope.allow')) if(!approvedAllow.has(entry)) fail(`CONTROL-APPROVAL-011 execution lock allow entry exceeds approval ${bound.id}: ${entry}`);
  }
  const currentCandidate=activeReq?.candidate_sha||activeReq?.implementation_sha;if(currentCandidate&&bound&&bound.candidate_sha!==currentCandidate)fail(`CONTROL-APPROVAL-005 approval ${bound.id} candidate_sha does not bind active candidate ${currentCandidate}`);
