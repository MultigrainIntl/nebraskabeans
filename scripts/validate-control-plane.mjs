...
for(const{name,record:a}of approvalFiles){
  ...
  const isLegacy=legacyApprovalIds.has(a.id);
  keys(a,['schema_version','id','authority','authorization_type','gate','authorized_at','base_sha','candidate_sha','scope','notes','source','countersigned_by','signature_provenance'],`approval.${name}`);
  required(a,['schema_version','id','authority','authorization_type','gate','authorized_at','base_sha','candidate_sha','scope'],`approval.${name}`);
  if(!isLegacy) required(a,['countersigned_by','signature_provenance'],`approval.${name}`);
  if(!isLegacy && (!nonEmpty(a.countersigned_by)||!nonEmpty(a.signature_provenance))) fail(`CONTROL-APPROVAL-010 ${name} missing countersign provenance`);
  keys(a.scope,['allow','prohibit'],`approval.${name}.scope`);
  const approvalAllow=list(a.scope?.allow,`approval.${name}.scope.allow`);
  const approvalProhibit=list(a.scope?.prohibit,`approval.${name}.scope.prohibit`);
  if(a.source)keys(a.source,['type','authority','statement'],`approval.${name}.source`);
  sha(a.base_sha,`approval.${name}.base_sha`);
  sha(a.candidate_sha,`approval.${name}.candidate_sha`,true);
  if(a.authority!=='GAJ')fail(`CONTROL-APPROVAL-001 ${name} authority must GAJ`);
  const productionClass=a.authorization_type==='production_scope'&&nonEmpty(a.countersigned_by)&&nonEmpty(a.signature_provenance);
  for(const rule of approvalAllow){
    if(productScopeRule(rule)&&!productionClass) fail(`CONTROL-SCOPE-004 ${name} product-path allow forbidden without countersigned production_scope: ${rule}`);
  }
