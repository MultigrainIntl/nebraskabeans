...
(()=>{
  ...
  function updateTruthPanel(area){
    const region=modelRegion(area),row=modelRow(area),sat=satelliteAt(area,state.date),official=officialAsOf(area.state,state.date),revision=acreageRevision(area.state,state.date),vegetation=vegetationSignal(sat,row?.stage),potential=potentialClass(region,row),production=productionScenario(area,state.date),released=false;/* RETIRED 17 Sep 2026. gisit-drybean-weather-ridge-v1 published a second, DIFFERENT pinto number on the same page as the season index — 2,418 lb/ac against the index's 2,100 for the same region on the same day — and carried the claim 'EXPERIMENTAL — OUT-OF-SAMPLE GAIN'. Its own validation block scores it against the historical MEDIAN, not a trend line, which is why it shows an 8.5% gain on 1 July. PLAN.md 1.3 records that exact figure as 'a straight line through 26 years of rising yields. Trend, not skill.' Its measured evidence below — soil moisture, NDVI, crop stage, rainfall — is untouched and still shown; only its YIELD output is withdrawn, so the site speaks one number. */
    ...
    if(released){
      const [low,high]=row.yield_interval_lb_ac,change=row.change_vs_historical_median_lb_ac;const band=high-low,dir=change>0?'above':change<0?'below':'at',cls=potential.replace(' POTENTIAL','');setText('plainAnswer',`${area.name}: ${cls.charAt(0)+cls.slice(1).toLowerCase()} pinto yield potential as of ${iso(state.date)}.`);setText('narrative',`Expected ${low.toLocaleString()}–${high.toLocaleString()} lb/ac — a ${band.toLocaleString()} lb/ac spread — running ${dir} the ${row.historical_median_lb_ac.toLocaleString()} lb/ac historical median. Built from selected-date weather and a base-50°F GDD stage model, calibrated against final NASS pinto outcomes. No current USDA yield forecast enters the equation.`);setText('yieldNow',cls);/* decision-workbench may override this with WITHHELD; that gate wins */setText('yieldRange',`${low.toLocaleString()}–${high.toLocaleString()} lb/ac · 80% band`);setText('yieldDelta',`${signed(change)} lb/ac vs the ${row.historical_median_lb_ac.toLocaleString()} lb/ac historical median`);setText('regionYield',`${row.yield_lb_ac.toLocaleString()} lb/ac`);setText('regionYieldWhy',`ESTIMATED by ${state.model.model.id}; selected-date hindcast MAE ${row.hindcast_mae_lb_ac} vs ${row.baseline_mae_lb_ac} lb/ac baseline.`);setText('yieldVsBase',`${signed(change)} lb/ac`);setText('confidence','NOT PROVEN');setText('confidenceWhy',`Tested county by county on 17 Sep 2026 against a plain trend line, using weather, satellite greenness and water-table depth. Five of seven state tests came out worse, two were too close to call. This estimate has never beaten a straight line through past yields. No confidence percentage invented.`);
    }else{
      setText('plainAnswer',`${area.name}: the yield estimate is the one at the top of this page.`);setText('narrative','This panel used to carry a second yield number from a different model. It disagreed with the estimate above — 2,418 against 2,100 lb/ac for the same region on the same day — and it was scored against the average of past years rather than against the trend those years were already following, which made it look better than it was. It has been withdrawn. Everything else on this panel is measured and still shown.');setText('yieldNow','SEE ABOVE');setText('yieldRange','One yield number per page. It is the season estimate at the top.');setText('yieldDelta','Second model withdrawn 17 Sep 2026');setText('regionYield','SEE ABOVE');setText('regionYieldWhy','The second yield model was withdrawn. The estimate at the top of the page is the only one.');setText('yieldVsBase','—');setText('confidence','NOT PROVEN');setText('confidenceWhy','Tested county by county on 17 Sep 2026 against a plain trend line, using weather, satellite greenness and water-table depth. Five of seven state tests came out worse, two were too close to call. No confidence percentage invented.');
    }
  ...
  function selectArea(id){
    stopPlayback();state.selected=id;$('region').value=id;
    /* THE DATA MUST NOT DEPEND ON THE ANIMATION. This line used to read
         map.flyTo(selectedArea().center,9,{duration:.6});refresh()
       and flyTo throws "Invalid LatLng object: (NaN, NaN)" — 369 times on a single live page
       load, and once per region change. The throw aborted selectArea BEFORE refresh() ran, so
       every panel below kept showing the PREVIOUS region while the picker showed the new one.
       Pick Southwest Nebraska, read Nebraska Panhandle's numbers. GAJ caught it in a screenshot;
       no automated check did, because nothing here throws on the server and the page still
       renders.
       The NaN comes from Leaflet's own projection: decision-workbench.js moves the map
       container into a new workspace and Leaflet caches the old container size, so until it is
       told the box changed its internal maths can produce NaN. invalidateSize() below fixes
       that cause. The try/catch is the belt: a failed camera move is cosmetic, a stale yield
       panel is a wrong number in front of a grower. refresh() runs either way. */
    try { map.invalidateSize(false); } catch (e) { /* map not ready yet */ }
    try { map.flyTo(selectedArea().center,9,{duration:.6}); }
    catch (e) { try { map.setView(selectedArea().center,9); } catch (e2) { /* cosmetic only */ } }
    refresh();
  }
